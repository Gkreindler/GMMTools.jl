...
# use prob.cache to store intermediate computation (no allocation) and store the model results
function sim_model(prob.data, prob.cache, theta)
    ...
end

# compute moments based on model results saved in prob.cache
function moments(prob.cache, theta)
end

function mom_fn(prob, theta)
    sim_model(prob.data, prob.cache, theta)
    return moments(prob.cache, theta)
end
```
